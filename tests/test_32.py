...
def test_password_uppercase_validation(browser):
    ...
    browser.find_element(By.ID, "password-confirm").send_keys("password1")
    browser.find_element(By.NAME, "register").click()
    time.sleep(5)

    # Проверить отображение сообщения об ошибке под полем "Пароль"
    error_message = browser.find_element(By.XPATH, "//span[text()='Пароль должен содержать хотя бы одну заглавную букву']").text
    assert error_message == "Пароль должен содержать хотя бы одну заглавную букву"
