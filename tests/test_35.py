...
def test_previous_password(browser):
    ...
    browser.find_element(By.ID, "password").send_keys("previouspassword")
    browser.find_element(By.ID, "password-confirm").send_keys("previouspassword")
    browser.find_element(By.NAME, "register").click()
    time.sleep(5)

    # Проверить отображение сообщения об ошибке под полем "Пароль"
    error_message = browser.find_element(By.XPATH, "//span[text()='Этот пароль уже использовался, укажите другой пароль']").text
    assert error_message == "Этот пароль уже использовался, укажите другой пароль"
